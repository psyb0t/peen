...
type RuntimeOptions struct {
	...
	// HookStateRoot is a writable runtime-owned root for session hook state. A
	// worker supplies its private socket mount because the config mount is
	// read-only.
