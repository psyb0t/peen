...
func newMessage(db *gorm.DB, opts ...gen.DOOption) message {
	...
	_message.SessionID = field.NewField(tableName, "session_id")
	_message.TurnID = field.NewField(tableName, "turn_id")
	_message.Sequence = field.NewInt64(tableName, "sequence")
	_message.Workspace = field.NewString(tableName, "workspace")
	_message.Role = field.NewString(tableName, "role")
	_message.Content = field.NewString(tableName, "content")
	_message.ModelID = field.NewString(tableName, "model_id")
...
func (m *message) updateTableName(table string) *message {
	...
	m.Workspace = field.NewString(table, "workspace")
...
func (m *message) fillFieldMap() {
	...
	m.fieldMap["workspace"] = m.Workspace
...
type IMessageDo interface {
	...
	CompletedHistory(sessionID uuid.UUID, afterSequence int64) (result []*models.Message, err error)
}
...
func (m messageDo) CompletedHistory(sessionID uuid.UUID, afterSequence int64) (result []*models.Message, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, sessionID)
	params = append(params, afterSequence)
	generateSQL.WriteString("SELECT m.* FROM messages AS m INNER JOIN turns ON turns.session_id = m.session_id AND turns.id = m.turn_id WHERE m.session_id = ? AND turns.state = 'completed' AND m.sequence > ? ORDER BY m.sequence ASC, m.id ASC ")
