...
to a known session through `X-Session-ID`; clients keep their own conversation
UUIDs and discover live activity through the global WebSocket feed. REST errors
use one envelope:
...
A client generates a canonical UUIDv4 for every new conversation and sends it
in `metadata.sessionId` on `message.send`. An existing UUID continues that
session. A new UUID is atomically created with its first accepted message. No
REST endpoint creates a session or accepts a user message. Peen gives each
socket a server-controlled [Aichteeteapee WShub](https://github.com/psyb0t/aichteeteapee)
identity, then fans each accepted session event to all global sockets and to
sockets filtered for that session. The server retains WShub's default origin
policy: an `Origin` header must match the request host outside explicitly
enabled local development mode.
...
```json
{
  ...
  "timestamp": 0,
  "metadata": {"sessionId": "canonical UUIDv4 generated by the client"},
  "triggeredBy": null
}
```
